...
def generate_pdf(code, stock_name, strategy_name, stats, annual,
    ...
    with pdf_backend.PdfPages(buf) as pdf:
        ...
        ax0.text(0.5, 0.08, f"Generated by Quant Platform v3.0",
...
if page == "📊 策略回测":
    ...
    if run_btn:
        ...
        st.subheader("📈 策略图表")
        st.image(_fig_to_buf(fig_main), width="stretch")

        st.subheader("📊 基准对比 & 年度收益")
        st.image(_fig_to_buf(fig_bench), width="stretch")
        ...
        if st.button("生成 PDF 报告", type="secondary"):
            with st.spinner("正在生成 PDF..."):
                fig_main_copy, _ = plt.subplots()
                plt.close(fig_main_copy)
